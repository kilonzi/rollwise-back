...
class MenuItemBase(BaseModel):
    ...
    category: str = Field(..., description="Category: Appetizer, Entree, Drink, Dessert")
    price: float = Field(..., ge=0, description="Selling price")
    allergens: Optional[str] = Field(None, description="Allergen information (e.g., contains nuts, dairy-free)")
    ...
    requires_age_check: bool = Field(False, description="Requires age verification (alcohol)")
...
class MenuItemFilter(BaseModel):
    ...
    requires_age_check: Optional[bool] = None
    has_discount: Optional[bool] = None
    search: Optional[str] = Field(None, description="Search in name, description, or ingredients")


class MenuItemBulkUpdate(BaseModel):
    """Schema for bulk updating MenuItems"""
    item_ids: List[str] = Field(..., min_items=1, description="List of MenuItem IDs to update")
    updates: MenuItemUpdate = Field(..., description="Updates to apply to all selected items")
