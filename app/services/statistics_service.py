...
from app.api.schemas.statistics_schemas import (
    AgentStatistics, ConversationStats, DurationStats, CallerStats,
    StatisticValue, MinutesStatistic, DateRangeInfo
)
...
class StatisticsService:
    ...
    def _get_period_stats(self, agent_id: str, start_date: datetime, end_date: datetime) -> Dict[str, Any]:
        ...
        voice_durations = self.db.query(Conversation.duration_seconds).filter(
            and_(
                Conversation.agent_id == agent_id,
                Conversation.conversation_type == "voice",
                Conversation.created_at >= start_date,
        ...
        unique_callers = self.db.query(Conversation.caller_phone).filter(
            and_(
                Conversation.agent_id == agent_id,
                Conversation.created_at >= start_date,
                Conversation.created_at <= end_date,
        ...
        current_callers = set(
            row[0] for row in self.db.query(Conversation.caller_phone).filter(
                and_(
                    Conversation.agent_id == agent_id,
                    Conversation.created_at >= start_date,
                    Conversation.created_at <= end_date,
    ...
    def _calculate_change(self, current: float, previous: float) -> Tuple[float, Optional[float]]:
    ...
    def _build_conversation_stats(self, current: Dict, previous: Dict) -> ConversationStats:
