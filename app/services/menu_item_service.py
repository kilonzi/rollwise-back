...
class MenuItemService:
    ...
    def create_menu_item(
        db: Session, agent_id: str, menu_item_data: MenuItemCreate
    ) -> MenuItem:
        ...
        try:
            ...
            if menu_item_data.number:
                ...
                if existing:
                    raise ValueError(
                        f"Menu item number '{menu_item_data.number}' already exists for this agent"
                    )

            # Create new menu item
            menu_item = MenuItem(agent_id=agent_id, **menu_item_data.model_dump())
    ...
    def get_menu_items(
        ...
        try:
            ...
            query = db.query(MenuItem).filter(
                MenuItem.agent_id == agent_id, MenuItem.active == True
            )
            ...
            if filters:
                ...
                if filters.is_limited_time is not None:
                    query = query.filter(
                        MenuItem.is_limited_time == filters.is_limited_time
                    )
                ...
                if filters.requires_age_check is not None:
                    query = query.filter(
                        MenuItem.requires_age_check == filters.requires_age_check
                    )
    ...
    def update_menu_item(
        db: Session, agent_id: str, item_id: str, updates: MenuItemUpdate
    ) -> MenuItem:
        """Update a menu item"""
        try:
            menu_item = (
            ...
            if updates.number and updates.number != menu_item.number:
                ...
                if existing:
                    raise ValueError(
                        f"Menu item number '{updates.number}' already exists for this agent"
                    )
    ...
    def bulk_update_menu_items(
        db: Session, agent_id: str, item_ids: List[str], updates: MenuItemUpdate
    ) -> List[MenuItem]:
        ...
        try:
            ...
            if len(menu_items) != len(item_ids):
                ...
                missing_ids = [
                    item_id for item_id in item_ids if item_id not in found_ids
                ]
            ...
            app_logger.info(
                f"Bulk updated {len(menu_items)} menu items for agent {agent_id}"
            )
            return menu_items
            ...
            db.rollback()
            app_logger.error(
                f"Error bulk updating menu items for agent {agent_id}: {str(e)}"
            )
            raise
    ...
    def get_menu_categories(db: Session, agent_id: str) -> List[str]:
        ...
        try:
            categories = (
                ...
                .filter(MenuItem.agent_id == agent_id, MenuItem.active == True)
                .distinct()
                .all()
            )
            ...
            app_logger.error(
                f"Error getting menu categories for agent {agent_id}: {str(e)}"
            )
            raise
    ...
    def toggle_availability(db: Session, agent_id: str, item_id: str) -> MenuItem:
        ...
        try:
            ...
            app_logger.info(
                f"Toggled availability for menu item {item_id} to {menu_item.available}"
            )
            return menu_item
            ...
            db.rollback()
            app_logger.error(
                f"Error toggling availability for menu item {item_id}: {str(e)}"
            )
            raise
