...
class CalendarService:
    ...
    def create_agent_calendar(self, agent_id: str, agent_name: str) -> str:
        ...
        try:
            ...
            app_logger.error(f"Failed to create calendar for agent {agent_id}: {str(e)}")
            raise
    ...
    def get_available_slots(self,
                          ...
                          start_date: datetime,
                          end_date: datetime,
                          slot_duration: Optional[int] = None) -> List[Dict[str, Any]]:
        ...
        business_hours = agent.business_hours or {
            "start": "09:00", "end": "17:00", "timezone": "UTC", "days": [1,2,3,4,5]
        }
            ...
            while current_date <= end_date_only:
                ...
                if current_date.weekday() + 1 in business_hours['days']:
                    day_slots = self._generate_day_slots(
                        current_date, business_hours, slot_duration,
                        agent.buffer_time, existing_events
                    )
    ...
    def _generate_day_slots(self,
        ...
        while current_slot + timedelta(minutes=slot_duration) <= end_datetime:
            ...
            for event in day_events:
                ...
                if (slot_start_with_buffer < event['end'] and slot_end_with_buffer > event['start']):
                    has_conflict = True
                    break
    ...
    def create_event(self,
        ...
        if not self._is_slot_available(agent, start_datetime, end_datetime):
            ...
            raise ValueError(f"Time slot conflicts with existing event. Suggested alternatives: {alternatives[:5]}")
    ...
    def _is_within_business_hours(self, agent: Agent, start_dt: datetime, end_dt: datetime) -> bool:
        ...
        return (start_time <= start_dt.time() <= end_time and
                start_time <= end_dt.time() <= end_time)

    def _is_slot_available(self, agent: Agent, start_dt: datetime, end_dt: datetime) -> bool:
    ...
    def update_event(self,
        ...
        try:
            ...
            for key, value in updates.items():
                if key in ['summary', 'description', 'location', 'start', 'end', 'attendees']:
                    current_event[key] = value

            # If updating time, validate business hours and conflicts
            if 'start' in updates or 'end' in updates:
                start_dt = datetime.fromisoformat(current_event['start']['dateTime'].replace('Z', '+00:00'))
                end_dt = datetime.fromisoformat(current_event['end']['dateTime'].replace('Z', '+00:00'))
